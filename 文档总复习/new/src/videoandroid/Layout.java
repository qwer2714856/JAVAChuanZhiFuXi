 ...
 * 
 * 
 * FrameLayout 							用的特别少，一般要求必须用的才用。  所有组件默认也是左对齐重叠在一起。
 * 这个对其方式和线性很像，但是线性有个ori的限制，但是这个没有。
 * android:layout_gravity="right|bottom",任何一个组件都是独立的。没有相对谁布局。也没有权重。
 * 
 * 
 * 
 * AbsoluteLayout						手机软件不会用到它，为什么因为没法做适配。 它有 android:layout_x="" android:layout_y="" 坐标问题才可以拖。						
 * 在分辨率固定的可以用它，或者少分辨率的，电视的可能会用到。
 * 
 * 
 * 单位
 * 
